...
export async function execute(interaction: ButtonInteraction): Promise<void> {
	...
	if (!interaction.customId.includes('update_append_roles:')) return;
	...
	const effect = await store.do(async (db) => {
		...
		if (!rewards.appendRoles.includes(selectedRoleId)) {
			if (rewards.appendRoles.length === 10) return;
			rewards.appendRoles.push(selectedRoleId);
		} else {
			content = `${roleMention(selectedRoleId)}を削除しました`;
			rewards.appendRoles = rewards.appendRoles.filter(
				(x) => x !== selectedRoleId,
			);
		}
	...
	if (!effect) {
		sendDeleteAfterMessage(
			{
				content: `${targetlvl}(${targetIsVcString})の付与ロールが既に10個設定されてるか、設定が見つからなかったためキャンセルしました`,
				sleepSecond: 15,
			},
			interaction,
		);
